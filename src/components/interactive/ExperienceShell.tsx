...
import { milestones, projects, skills, type Milestone, type Project, type SpatialSection } from '@/data/portfolio';
  ...
  return (
    <div className={`noise relative overflow-x-clip ${isLight ? 'bg-[#dfe5dc] text-ink' : 'bg-ink text-paper'}`}>
      ...
      <main className="relative z-10">
        ...
        <section id="timeline" data-section="timeline" className="min-h-screen px-6 py-28 sm:px-10 lg:px-16"><div className="mx-auto max-w-[1500px]"><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-signal">03 / progression</p><h2 className="mt-4 max-w-2xl font-display text-6xl leading-[0.9] text-paper sm:text-8xl">The line is<br /><em className="text-paper/40">still moving.</em></h2><div className="mt-20 grid gap-0 border-t border-white/15 md:grid-cols-4">{milestones.map((milestone) => <button key={milestone.year} type="button" onClick={() => { setSelectedMilestone(milestone); setSelectedProject(undefined); }} className="group border-b border-white/15 py-6 text-left md:border-b-0 md:border-r md:px-6 md:first:pl-0"><span className="font-mono text-xs text-signal">{milestone.year}</span><span className="my-8 block h-2 w-2 rounded-full bg-paper transition-colors group-hover:bg-signal" /><h3 className="font-display text-3xl text-paper">{milestone.title}</h3><p className="mt-3 text-sm leading-6 text-paper/45">{milestone.detail}</p></button>)}</div></div></section>
        ...
        <section id="contact" data-section="contact" className="flex min-h-[70vh] items-end px-6 py-28 sm:px-10 lg:px-16"><div className="mx-auto w-full max-w-[1500px] border-t border-white/15 pt-10"><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-signal">05 / open channel</p><div className="mt-8 flex flex-col justify-between gap-12 md:flex-row md:items-end"><h2 className="font-display text-7xl leading-[0.8] text-paper sm:text-9xl">Let&apos;s make<br /><em className="text-signal">something</em><br />clear.</h2><div className="max-w-xs text-sm leading-6 text-paper/50"><p>Have a product, system, or strange idea that needs a shape?</p><a className="mt-6 inline-block border-b border-signal pb-1 font-mono text-[10px] uppercase tracking-[0.2em] text-signal" href="mailto:meeharshu8686@gmail.com">meeharshu8686@gmail.com ↗</a></div></div></div></section>
      </main>
      <DetailPanel project={selectedProject} milestone={selectedMilestone} onClose={() => { setSelectedProject(undefined); setSelectedMilestone(undefined); }} />
